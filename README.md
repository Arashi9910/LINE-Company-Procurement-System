...
`取消補貨` 只接受仍全部處於「待確認」的補貨單，限原申請人或已啟用的管理員使用。`取消採購` 只接受尚未開始到貨且所有有效品項皆為「已下單」的補貨單，限已啟用的 `採購確認` 或 `管理員` 使用；它只取消補貨系統紀錄，採購平台上的訂單仍需另外取消。授權指令只有已啟用的管理員能使用。系統使用 LINE Webhook 提供的 user ID 識別成員，不依賴可能重複或變更的顯示名稱。

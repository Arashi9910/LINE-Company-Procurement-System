...
- 使用既有 Cloud Run 服務帳號與 Job `roles/run.invoker`，不新增 npm 套件。

## Phase 1：商品主檔自動同步

- [ ] Task 1：新增 `syncCatalogItems`
...
- [ ] 飛鼠自動主檔測試通過。
- [ ] review/read-only 行為維持。

## Phase 2：LINE 非同步指令

- [ ] Task 3：建立 Cloud Run Job runner
...
- [ ] 單一 LINE 事件只會建立一個同步要求。
- [ ] 非管理員、重送、已有執行中工作皆不重複啟動。

## Phase 3：文件與完整驗證

- [ ] Task 5：更新 README、部署與維運文件。
- [ ] Task 6：執行 `npm.cmd test`、`npm.cmd run lint`、`npm.cmd run build`、`git diff --check`。
- [ ] Task 7：建立原子提交；正式部署與執行另行取得同意。
...
| 正式 mode 切換影響每日排程 | 保留 review mode；部署與首次 live 同步分開核准 |

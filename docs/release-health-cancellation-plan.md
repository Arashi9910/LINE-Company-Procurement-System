...
- `/health` 只代表程序存活；`/ready` 透過唯讀 Sheet 請求代表核心資料相依可用。
...
- [ ] Task 2：核心相依 fail-fast 與 Sheet readiness
...
- [ ] Task 3：部署腳本綁定 Git 與 Cloud Run probes
...
- [ ] 健康與部署腳本測試通過。
- [ ] 不呼叫正式 Google Cloud 或 Google Sheets。
